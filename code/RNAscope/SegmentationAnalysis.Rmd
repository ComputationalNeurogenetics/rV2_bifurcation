...
ggplot(slided.tb, aes(x=cell.distances,y=count.dots)) + geom_point(aes(colour = factor(ch))) + scale_color_manual(values=c("magenta2", "dodgerblue", "forestgreen"),labels=c("Tal1","Sox4","Insm1")) + labs(color="Channel")  + theme_minimal() +ggtitle("Sliding mean (5) values") + geom_smooth(aes(colour = factor(ch)),method = "loess") + xlab("Micrometers") + ylab("Mean Intensity") + theme(axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14), axis.title.x=element_text(size = 14),axis.title.y=element_text(size = 14)) + scale_x_continuous(breaks = seq(0, max(slided.tb$cell.distances), by = 100)) + ylim(0, NA)
``` 
