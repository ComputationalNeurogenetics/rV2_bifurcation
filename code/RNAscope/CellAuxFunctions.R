...
plot_sliding_mean_replicates <- function(data, 
  ...
  # Create the ggplot with optional points and a fitted line per channel
  p <- ggplot(sliding_means, aes(x = cell.distances, y = count.dots, 
                 colour = factor(channel_label))) + 
  {if (draw_points) geom_point(aes(shape = replicate), size = 1.25) else NULL} +  # Conditionally add points
