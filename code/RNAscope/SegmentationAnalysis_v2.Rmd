...
```{r Image paths}
rois.path <- "~/Workspace/rV2_bifurcation/code/RNAscope/TIFF/"
fluor.channel.path <- "~/Workspace/rV2_bifurcation/code/RNAscope/fluor_channels/"
plot_loc<-"~/OneDrive - University of Helsinki/E12R1 project/Manuscript I - Regulation of Tal1 dependent rV2 lineage bifurcation/Figures/plots/Fig4/"
```

```{r}
image_bases <- image_bases <- c(
  "e3_1a_sb514_e12.5_63x", "e3_2a_d611_E12.5_63x", "e3_1a_sb510_e12.5_63x",
  "e3b_1b_sb514_e12.5_63x", "e3b_3b_sb509_e12.5_63x", "e3b_1b_sb510_e12.5_63x",
  "e4_1c_sb514_e12.5_63x", "e4_2b_sb0488_e12.5_63x", "e4_4c_sb0479_e12.5_63x"
)
...
count.per.cell.channel.filt.e3_1a_sb514 <- process_image_data(
  ...
  channel_labels = c("Tal1","Sox4","Insm1")
)
```
...
band.trail.3 <- "_Sox4.tif"
dapi.trail <- "_dapi.tif"
image.base <- "e3_2a_d611_E12.5_63x"
```

```{r Reading imageJ ROIS file for a image e3_2a_d611}
rois <- read.ijzip(file = paste(rois.path,image.base,rois.trail,sep=""))
rois.ss <- ij2spatstat(rois)
```

```{r Reading channels from image files e3_2a_d611, warning=FALSE}
ras.1 <- raster(paste(fluor.channel.path,image.base,band.trail.1,sep=""), band=1)
ras.2 <- raster(paste(fluor.channel.path,image.base,band.trail.3,sep=""), band=1)
ras.3 <- raster(paste(fluor.channel.path,image.base,band.trail.2,sep=""), band=1)
...
```{r Plot DAPI channel to check orientation e3_2a_d611}
...
```{r Plot channel 1 to check orientation e3_2a_d611}
plot(im.1)
plot(rois, TRUE,xlim=c(0, ras.1@extent@xmax), ylim=c(ras.1@extent@ymax, 0))
```

```{r Plot channel 2 to check orientation e3_2a_d611}
plot(im.2)
plot(rois, TRUE,xlim=c(0, ras.2@extent@xmax), ylim=c(ras.2@extent@ymax, 0))
```

```{r Plot channel 3 to check orientation e3_2a_d611}
plot(im.3)
plot(rois, TRUE,xlim=c(0, ras.3@extent@xmax), ylim=c(ras.3@extent@ymax, 0))
```

```{r Define circle e3_2a_d611, eval=FALSE}
plot(im.dapi)
three.points <- locator()
saveRDS(three.points,file="../../analysis/e3_2a_d611.three.points.Rds")
```

```{r}
three.points <- readRDS(file="../../analysis/e3_2a_d611.three.points.Rds")
```

```{r Process e3_2a_d611}
count.per.cell.channel.filt.e3_2a_d611 <- process_image_data(
  rois.ss = rois.ss, 
  im.1 = im.1, 
  im.2 = im.2, 
  im.3 = im.3, 
...
```{r}
ggplot(count.per.cell.channel.filt.e3_2a_d611, aes(x=cell.distances,y=count.dots)) + geom_point(aes(colour = factor(ch))) + scale_color_manual(values=c("magenta2", "dodgerblue", "forestgreen"),labels=c("Tal1","Sox4","Insm1")) + labs(color="Channel") + theme_minimal() +ggtitle("Raw values") + xlab("Micrometers")
```

```{r}
plot_sliding_mean(count.per.cell.channel.filt.e3_2a_d611, window_size = 8)
```
...
```{r fig.width=12, fig.height=4}
ggplot(count.per.cell.channel.filt.e3_2a_d611, aes(fill=as.character(ch), y=norm_count_dots, x=cell.distances)) + geom_bar(position="stack", width = 1,stat="identity") + scale_fill_manual(values=c("magenta2", "dodgerblue", "forestgreen"),labels=c("Tal1","Sox4","Insm1")) + theme_minimal()  + theme(axis.text.x = element_blank()) + labs(fill="Channel") + xlab("Cells in increasing distance from VZ (as units of average cell size) from VZ (as units of average cell size)")
```
...
combined_replicates <- rbind(count.per.cell.channel.filt.e3_1a_sb510, 
                             count.per.cell.channel.filt.e3_2a_d611, 
                             count.per.cell.channel.filt.e3_1a_sb514)

combined_replicates <- filter_by_max_distance(combined_replicates)

p1 <- plot_sliding_mean_replicates(combined_replicates, window_size = 12, use_cell_normalized = FALSE, normalize_per_channel=TRUE, align_channels_across_replicates = TRUE,draw_points = FALSE)

p1[[1]]
pdf(file=paste(plot_loc,"e3.aggregated.pdf",sep=""), width = 8, height = 6)
p1[[1]]
dev.off()
```
...
# gam_model_both_updated <- gam(count.dots ~ replicate + ch + 
...
count.per.cell.channel.filt.e3b_1b_sb514 <- process_image_data(
  ...
  quantile_threshold = 0.25,
  cell.size.avg = global_avg_diameter, 
  channel_labels = c("Tal1","Tead2","E2f1")
)
```
...
p1 <- plot_sliding_mean_replicates(combined_replicates, window_size = 12, use_cell_normalized = FALSE, normalize_per_channel=TRUE, align_channels_across_replicates = TRUE, exclude_channels=2,draw_points = FALSE)

p1[[1]]
pdf(file=paste(plot_loc,"e3b.aggregated.pdf",sep=""), width = 8, height = 6)
p1[[1]]
dev.off()
```
...
count.per.cell.channel.filt.e4_1c_sb514 <- process_image_data(
  ...
  quantile_threshold = 0.25,
  cell.size.avg = global_avg_diameter, 
  channel_labels = c("Tal1","Tead2","Ebf1")
)
```
...
band.trail.3 <- "_Ebf1.tif"
dapi.trail <- "_dapi.tif"
image.base <- "e4_1b_d611_E12.5_63x"
```

```{r Reading imageJ ROIS file for a image e4_1b_d611}
rois <- read.ijzip(file = paste(rois.path,image.base,rois.trail,sep=""))
rois.ss <- ij2spatstat(rois)
```

```{r Reading channels from image files e4_1b_d611, warning=FALSE}
ras.1 <- raster(paste(fluor.channel.path,image.base,band.trail.1,sep=""), band=1)
ras.2 <- raster(paste(fluor.channel.path,image.base,band.trail.2,sep=""), band=1)
ras.3 <- raster(paste(fluor.channel.path,image.base,band.trail.3,sep=""), band=1)
ras.dapi <- raster(paste(fluor.channel.path,image.base,dapi.trail,sep=""), band=1)
...
```{r Plot DAPI channel to check orientation e4_1b_d611}
#plot(NA,NA,asp=1, xlim=c(0, ras.1@extent@xmax), ylim=c(ras.1@extent@ymax, 0))
plot(im.dapi)
plot(rois, TRUE,xlim=c(0, ras.1@extent@xmax), ylim=c(ras.1@extent@ymax, 0))
```
...
```{r Plot channel 1 to check orientation e4_1b_d611}
#plot(NA,NA,asp=1, xlim=c(0, ras.1@extent@xmax), ylim=c(ras.1@extent@ymax, 0))
plot(im.1)
plot(rois, TRUE,xlim=c(0, ras.1@extent@xmax), ylim=c(ras.1@extent@ymax, 0))
```

```{r Plot channel 2 to check orientation e4_1b_d611}
plot(im.2)
plot(rois, TRUE,xlim=c(0, ras.2@extent@xmax), ylim=c(ras.2@extent@ymax, 0))
```

```{r Plot channel 3 to check orientation e4_1b_d611}
plot(im.3)
plot(rois, TRUE,xlim=c(0, ras.3@extent@xmax), ylim=c(ras.3@extent@ymax, 0))
```
...
```{r Define circle e4_1b_d611, eval=FALSE}
plot(im.dapi)
three.points <- locator()
saveRDS(three.points,file="../../analysis/e4_1b_d611.three.points.Rds")
```

```{r}
three.points <- readRDS(file="../../analysis/e4_1b_d611.three.points.Rds")
```

```{r Process e4_1b_d611}
count.per.cell.channel.filt.e4_1b_d611 <- process_image_data(
  rois.ss = rois.ss, 
  im.1 = im.1, 
  im.2 = im.2, 
  im.3 = im.3, 
...
```{r}
ggplot(count.per.cell.channel.filt.e4_1b_d611, aes(x=cell.distances,y=count.dots)) + geom_point(aes(colour = factor(ch))) + scale_color_manual(values=c("magenta2", "dodgerblue", "forestgreen"),labels=c("Tal1","Tead2","Ebf1")) + labs(color="Channel") + theme_minimal() +ggtitle("Raw values") + xlab("Micrometers")
```

```{r}
plot_sliding_mean(count.per.cell.channel.filt.e4_1b_d611,window_size = 12)
```

## Stacked barplot of fluor dot proportions as distance from VZ

```{r fig.width=12, fig.height=4}
ggplot(count.per.cell.channel.filt.e4_1b_d611, aes(fill=as.character(ch), y=norm_count_dots, x=cell.distances)) + geom_bar(position="stack", width = 1,stat="identity") + scale_fill_manual(values=c("magenta2", "dodgerblue", "forestgreen"),labels=c("Tal1","Tead2","Ebf1")) + theme_minimal()  + theme(axis.text.x = element_blank()) + labs(fill="Channel") + xlab("Cells in increasing distance from VZ (as units of average cell size)") + xlab("Micrometers")
```
...
```{r}
plot_sliding_mean(count.per.cell.channel.filt.e4_4c_sb0479,window_size = 12)
```
...
combined_replicates <- filter_by_max_distance(combined_replicates, extend.factor = 1.5)

p1 <- plot_sliding_mean_replicates(combined_replicates, window_size = 12, use_cell_normalized = FALSE, normalize_per_channel=TRUE, align_channels_across_replicates = TRUE,draw_points = FALSE)

p1[[1]]
pdf(file=paste(plot_loc,"e4.aggregated.pdf",sep=""), width = 8, height = 6)
p1[[1]]
dev.off()
```
...
```{r}
save.image(file="SegmentationAnalysisImage121124.RData")
```
