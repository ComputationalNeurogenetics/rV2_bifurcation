...
DEG.res.1 <- FindMarkers(rV2.dataset, ident.1="GA1_2", ident.2 = "GL1_2",logfc.threshold=0, assay = "RNA", slot="data", mean.fxn=mean.fxn) %>% rownames_to_column(var="ensg_id") %>% as_tibble()# %>% dplyr::filter(p_val_adj<0.01)
...
DEG.res.2 <- FindMarkers(merged.datasets, ident.1="GA1_2", ident.2 = "SR2_3",logfc.threshold=0, assay = "RNA", slot="data", mean.fxn=mean.fxn) %>% rownames_to_column(var="ensg_id") %>% as_tibble() #%>% dplyr::filter(p_val_adj<0.01)

colnames(assay.averages.merged$RNA) <- str_replace_all(colnames(assay.averages.merged$RNA), pattern = "-", replacement = "_")

DEG.res.2$SR2_3_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.2$ensg_id,"SR2_3"]+1,base=2)
DEG.res.2$GA1_2_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.2$ensg_id,"GA1_2"]+1, base=2)
DEG.res.2$SR2_3_exp_avg <- assay.averages.merged$RNA[DEG.res.2$ensg_id,"SR2_3"]
...
```{r Correlations between target genes and selector genes, warning=FALSE, eval=TRUE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
...
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.3[[target]]$targets,DEG.res=DEG.res.1,TF.name=target, comp.title="GA/GL DEG")}))
...
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.3[[target]]$targets,DEG.res=DEG.res.2,TF.name=target,comp.title="GA/SR DEG")}))
