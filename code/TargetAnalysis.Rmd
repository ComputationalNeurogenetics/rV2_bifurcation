...
## With C&T False and Cons True ft True - Set1

```{r Set1}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
targetGenes.set.1 <- lapply(TF_names,getTargets,CT=FALSE, Cons=TRUE, ft=TRUE)
names(targetGenes.set.1) <- TF_names
sapply(targetGenes.set.1,function(p){length(p$targets)})
```

## With C&T True and Cons False ft True - Set 2
```{r Set2}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
targetGenes.set.2 <- lapply(TF_names,getTargets,CT=TRUE, Cons=FALSE, ft=TRUE)
names(targetGenes.set.2) <- TF_names
sapply(targetGenes.set.2,function(p){length(p$targets)})
```

## With C&T True and Cons True ft True - Set3
...
```{r}
combined.target.counts <- rbind(sapply(targetGenes.set.1,function(p){length(p$targets)}),sapply(targetGenes.set.2,function(p){length(p$targets)}),sapply(targetGenes.set.3,function(p){length(p$targets)}),sapply(targetGenes.set.4,function(p){length(p$targets)}))
...
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.1[[target]]$targets,DEG.res=DEG.res,TF.name=target, comp.title="GA/GL DEG")}))

invisible(sapply(set1.GSEA,function(g){print(g$p1)}))

tmp.1 <- lapply(targetGenes.set.1,function(p){p$targets})
names(tmp.1) <- names(targetGenes.set.1)
...
DEG.res.subset <- lapply(targetGenes.set.1, function(p){
...
create_dt(filter(combined_tibble, TF=="Sox4"))
create_dt(filter(combined_tibble, TF=="Klf17"))
create_dt(filter(combined_tibble, TF=="Myc"))
```
...
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.1[[target]]$targets,DEG.res=DEG.res.GASR,TF.name=target,comp.title="GA/SR DEG")}))
...
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.2[[target]]$targets,DEG.res=DEG.res,TF.name=target, comp.title="GA/GL DEG")}))

invisible(sapply(set1.GSEA,function(g){print(g$p1)}))

tmp.1 <- lapply(targetGenes.set.2,function(p){p$targets})
names(tmp.1) <- names(targetGenes.set.2)
results <- fgsea(pathways = tmp.1, 
                 stats = DEG.res.GSEA, 
                 minSize = 5,  # Minimum size of a gene set to be considered
                 maxSize = 5000) # Maximum size of a gene set to be considered
...
DEG.res.subset <- lapply(targetGenes.set.2, function(p){
  left_join(filter(DEG.res, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
...
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.2[[target]]$targets,DEG.res=DEG.res.GASR,TF.name=target,comp.title="GA/SR DEG")}))
...
```{r GA/GL GSEA for Set 4, warning=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.4[[target]]$targets,DEG.res=DEG.res,TF.name=target, comp.title="GA/GL DEG")}))

invisible(sapply(set1.GSEA,function(g){print(g$p1)}))

tmp.1 <- lapply(targetGenes.set.4,function(p){p$targets})
names(tmp.1) <- names(targetGenes.set.4)
results <- fgsea(pathways = tmp.1, 
                 stats = DEG.res.GSEA, 
                 minSize = 5,  # Minimum size of a gene set to be considered
                 maxSize = 5000) # Maximum size of a gene set to be considered
...
DEG.res.subset <- lapply(targetGenes.set.4, function(p){
  left_join(filter(DEG.res, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
...
```{r GA/SR GSEA for Set 4, warning=FALSE, echo=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.4[[target]]$targets,DEG.res=DEG.res.GASR,TF.name=target,comp.title="GA/SR DEG")}))
...
```{r}
Tal1.tb.data <- as_tibble(dbGetQuery(con, 'SELECT tb.TF_gene_name, tb.PRO1_2_score, tb.CO1_2_score, tb.GA1_2_score, tb.PRO1_2_bound, tb.CO1_2_bound, tb.GA1_2_bound FROM tobias as tb WHERE tb.TF_gene_name IN ("TAL1","GATA2","GATA3","VSX2")'))
Tal1.tb.data
```

```{r}
results <- Tal1.tb.data %>% dplyr::select(PRO1_2_bound, CO1_2_bound, GA1_2_bound) %>% 
  mutate(
...
```{r}
write_csv(Tal1.tb.data, file = "tb.data.csv")
```


```{r}
Tal1.tb.data.long <- Tal1.tb.data %>% dplyr::select(PRO1_2_score, CO1_2_score, GA1_2_score) %>% pivot_longer(cols = everything(), names_to = "Cell_group", values_to = "Score") 
Tal1.tb.data.long$Cell_group <- factor(Tal1.tb.data.long$Cell_group, levels = c("PRO1_2_score", "CO1_2_score","GA1_2_score"))

ggplot(Tal1.tb.data.long,aes(y = Score)) + geom_boxplot(aes(x = Cell_group))
```
...
# Pre-defined chosen minima for each TF and cell group
chosen_minima <- list(
  GATA2 = c(PRO1_2_score = 1.353, CO1_2_score = 0.081, GA1_2_score = 2.314),
  GATA3 = c(PRO1_2_score = 1.169, CO1_2_score = 0.075, GA1_2_score = 0.057),
  TAL1 = c(PRO1_2_score = 1.944, CO1_2_score = 0.079, GA1_2_score = 0.048),
  VSX2 = c(PRO1_2_score = 1.215, CO1_2_score = 0.086, GA1_2_score = 1.897)
)

# Function to plot KDE and the chosen minima for a given TF
plot_kde_with_minima <- function(tf_name, data, chosen_minima) {
  tf_data <- data %>% filter(TF_gene_name == tf_name)
  
  # Creating a list of plots
  plots <- list()
  
  for (score_column in names(chosen_minima[[tf_name]])) {
    p <- ggplot(tf_data, aes_string(x = score_column)) +
      geom_density(fill = "blue", alpha = 0.4) +
      geom_vline(xintercept = chosen_minima[[tf_name]][[score_column]], 
                 color = "green", linetype = "solid", size = 1.5) +
      labs(title = paste("KDE for", tf_name, ":", score_column),
           x = "Score", y = "Density") +
      theme_minimal() + xlim(c(0,4))
    
    plots[[score_column]] <- p
  }
  
  return(plots)
}

# Example usage: Plot for TAL1
tal1_plots <- plot_kde_with_minima("TAL1", data, chosen_minima)

# Print plots for TAL1
if (!is.null(tal1_plots$PRO1_2_score)) print(tal1_plots$PRO1_2_score)
if (!is.null(tal1_plots$CO1_2_score)) print(tal1_plots$CO1_2_score)
if (!is.null(tal1_plots$GA1_2_score)) print(tal1_plots$GA1_2_score)
