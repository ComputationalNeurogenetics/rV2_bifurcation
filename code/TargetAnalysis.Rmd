...
target.genes.in.order <- names(sort(DEG.res.GSEA[targetGenes.set.3[["Tal1"]]$targets], decreasing = TRUE))
Targets.expression.avg <- t(assay.averages$RNA[sapply(target.genes.in.order,function(id){gene_name2id[[id]]}),c("PRO1_2","CO1_2","GA1_2","GL1_2")])
colnames(Targets.expression.avg) <- sapply(colnames(Targets.expression.avg),function(id){gene_id2name[[id]]})
col_fun = colorRamp2(c(min(Targets.expression.avg), mean(Targets.expression.avg),max(Targets.expression.avg)), c("white", "red","darkred"))

Heatmap(as.matrix(Targets.expression.avg), cluster_columns = FALSE,heatmap_width = unit(12, "inches"), col=col_fun, show_column_names = FALSE)
