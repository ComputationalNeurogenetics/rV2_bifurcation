...
getTargets <- function(TF_name, CT, Cons){
  if (!CT){
...
do.GSEA <- function(targets,DEG.res,TF.name){
  lower.thr <- .25
  ...
  p1 <- plotEnrichment(targets, DEG.res.GSEA) + ggtitle(paste(TF.name,"target enrichments in GA/GL DEG")) + geom_rect(aes(xmin=0,xmax=lower.pos,ymin=-0.05,ymax=0.05), fill="blue", alpha=0.002) + geom_rect(aes(xmin=mid.lower.index,xmax=mid.upper.index,ymin=-0.05,ymax=0.05), fill="green", alpha=0.002) + geom_rect(aes(xmin=upper.pos,xmax=xmax.right,ymin=-0.05,ymax=0.05), fill="red", alpha=0.002)
...
```{r}
combined.target.counts <- rbind(sapply(targetGenes.set.1,length),sapply(targetGenes.set.2,length),sapply(targetGenes.set.3,length))

rownames(combined.target.counts) <- c("CT False and Cons True","CT True and Cons False","CT True and Cons True")
...
```{r Calculating averages and DEG, include=FALSE}
...
```{r GSEA for Set 1, warning=FALSE, echo=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2","Sox4","Klf17","Myc")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.1[[target]],DEG.res=DEG.res,TF.name=target)}))
...
```{r GSEA for Set 2, warning=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
set2.GSEA <- lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.2[[target]],DEG.res=DEG.res,TF.name=target)})
...
```{r GSEA for Set 3, warning=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
set3.GSEA <- lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.3[[target]],DEG.res=DEG.res,TF.name=target)})
