...
```{r Reading Seurat dataobject, eval=FALSE}
...
```{r Example of fetching factors give group, chr, start and end}
test.tal1 <- extract.factors(db.name = "~/Workspace/TOBIAS.dr.h12_2.sqlite", group_name = "CO1_2", chr="chr4",start=114986077,end=115134633)

test.gata2 <- extract.factors(db.name = "~/Workspace/TOBIAS.dr.h12_2.sqlite", group_name = "CO1_2", chr="chr6",start=88103987,end=88301611)

test.gata3 <- extract.factors(db.name = "~/Workspace/TOBIAS.dr.h12_2.sqlite", group_name = "CO1_2", chr="chr2",start=9834965,end=9996391)
...
test.tal1 <- as_tibble(dbGetQuery(con.obj, 'SELECT tb.* FROM tobias as tb,feature as ft, exp as exp WHERE ft.seqnames="chr4" AND ft.start>=114986077 AND ft.end<=115134633 AND tb.features==ft.feature AND mean_cons>0.5 AND exp.ensg_id=tb.ensg_id AND (tb.GA1_2_bound=1) AND (exp.GA1_2>1.2)'))
