...
```{r, message=FALSE}
all.genes <- dbGetQuery(con.obj, 'SELECT DISTINCT gene_name FROM gene_metadata as gm, links as li WHERE li.ensg_id=gm.ensg_id;')[,1]

TF.groups.z2 <- mclapply(1:length(all.genes),function(i){
  gene_name <- all.genes[i]
  #message_parallel(paste(i,gene_name,sep = ": "))
  extract.factors(db.name = "~/Workspace/TOBIAS.dr.h12_210324.sqlite", group_name = "GA1_2", gene_name = gene_name, zscore.thr=2)
}, mc.cores = 8)

```

```{r}
# Assuming 'my_list' is your list of character vectors
# my_list <- list(c("A", "B"), c("A", "C"), c("B", "C", "A"), ...)
...
pairs_df <- map_df(TF.groups.z2, get_pairs, .id = "source")
...
prop.matrix <- round(frequency_matrix/length(TF.groups.z2), digits = 3)
