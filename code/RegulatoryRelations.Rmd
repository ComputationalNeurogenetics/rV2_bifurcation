...
TF.groups.z2_ac0 <- mclapply(1:length(all.genes),function(i){
  ...
  extract.factors(db.name = "~/Workspace/TOBIAS.dr.h12_210324.sqlite", group_name = "GA1_2", gene_name = gene_name, zscore.thr=2, acc.thr = 0)
}, mc.cores = 8)
...
num_TFs_per_gene <- c(29, 29, 23)  # For genes 1, 2, and 3, respectively
total_TFs <- 950  # Total pool of unique TFs
C <- 3  # Observed number of common TFs across all genes
...
cat("Estimated probability of observing at least", C, "common TFs across genes:", prob_at_least_C, "\n")
```
