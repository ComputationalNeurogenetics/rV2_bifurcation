...
do.GSEA <- function(targets,DEG.res,TF.name, comp.title){
  ...
  # 
  limits <- list(upper.limit=upper.limit,lower.limit=lower.limit,mid.lower.limit=mid.lower.limit,mid.upper.limit=mid.upper.limit,upper.pos=upper.pos,lower.pos=lower.pos)
...
for (i in 1:length(set1.GSEA)){
    h1 <- drawTargetLinePlot(DEG.res=set1.GSEA[[i]], limits.to.plot, cell.groups=c("SR1","SR2"))
    p1 <- set1.GSEA[[i]]$p1
  print(plot_grid(plotlist = list(p1, h1), labels = c('A', 'B'), label_size = 12, ncol = 1, nrow = 2))
}
```
