...
DEG.res.subset <- lapply(targetGenes.set.3, function(p){
  left_join(filter(DEG.res.1, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
...
DEG.res.subset <- lapply(targetGenes.set.3, function(p){
  left_join(filter(DEG.res.2, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
