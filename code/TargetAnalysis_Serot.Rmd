---
title: "R Notebook for selector gene target analysis in Serot"
...
drawTargetLinePlot <- function(DEG.res, target.genes, assay.averages,gene_name2id, gene_id2name,limits.to.plot, cell.groups){
  
  # Just take DEG res with p-value filter, pivot long and create geom_line plot with group colros
...
do.GSEA <- function(targets,DEG.res,TF.name, comp.title){
  ...
  p1 <- plotEnrichment(targets, DEG.res.GSEA) + ggtitle(paste(TF.name,"target enrichments in", comp.title)) + geom_rect(aes(xmin=0,xmax=upper.pos,ymin=-0.05,ymax=0.05), fill="blue", alpha=0.002) +
geom_rect(aes(xmin=lower.pos,xmax=xmax.right,ymin=-0.05,ymax=0.05), fill="red", alpha=0.002)
#  geom_rect(aes(xmin=mid.lower.index,xmax=mid.upper.index,ymin=-0.05,ymax=0.05), fill="green", alpha=0.002) + 
...
cores <- 6
con <- DBI::dbConnect(RSQLite::SQLite(), dbname = "~/Workspace/TOBIAS.serot.h12_230524.sqlite")
...
targetGenes.set.3 <- lapply(TF_names,getTargets_SR,CT=TRUE, Cons=TRUE, ft=TRUE)
...
DEG.res.1 <- FindMarkers(merged.datasets, ident.1="SR1", ident.2 = "SR2",logfc.threshold=0, assay = "RNA", slot="data", mean.fxn=mean.fxn) %>% rownames_to_column(var="ensg_id") %>% as_tibble()# %>% dplyr::filter(p_val_adj<0.01)

colnames(assay.averages$RNA) <- str_replace_all(colnames(assay.averages$RNA), pattern = "-", replacement = "_")

DEG.res.1$SR2_exp_avg_log2 <- log(assay.averages$RNA[DEG.res.1$ensg_id,"SR2"]+1,base=2)
DEG.res.1$SR1_exp_avg_log2 <- log(assay.averages$RNA[DEG.res.1$ensg_id,"SR1"]+1, base=2)
DEG.res.1$SR2_exp_avg <- assay.averages$RNA[DEG.res.1$ensg_id,"SR2"]
DEG.res.1$SR1_exp_avg <- assay.averages$RNA[DEG.res.1$ensg_id,"SR1"]
...
```{r Calculating averages and DEG for SR2/SR3 axis, include=FALSE}
assay.averages <- AverageExpression(merged.datasets,group.by = "merged.groups")

Idents(merged.datasets) <- merged.datasets$merged.groups
DEG.res.2 <- FindMarkers(merged.datasets, ident.1="SR2", ident.2 = "SR3",logfc.threshold=0, assay = "RNA", slot="data", mean.fxn=mean.fxn) %>% rownames_to_column(var="ensg_id") %>% as_tibble()# %>% dplyr::filter(p_val_adj<0.01)

colnames(assay.averages$RNA) <- str_replace_all(colnames(assay.averages$RNA), pattern = "-", replacement = "_")

DEG.res.2$SR2_exp_avg_log2 <- log(assay.averages$RNA[DEG.res.2$ensg_id,"SR2"]+1,base=2)
DEG.res.2$SR3_exp_avg_log2 <- log(assay.averages$RNA[DEG.res.2$ensg_id,"SR3"]+1, base=2)
DEG.res.2$SR2_exp_avg <- assay.averages$RNA[DEG.res.2$ensg_id,"SR2"]
DEG.res.2$SR3_exp_avg <- assay.averages$RNA[DEG.res.2$ensg_id,"SR3"]
...
SR2.subset <- subset(merged.datasets, idents=c("SR2"))
...
# SR2.correlations 
expression_matrix <- GetAssayData(SR2.subset, slot = "data")
TF.cors.SR2 <- lapply(TF_names,function(TF){
    correlations <- mclapply(1:nrow(expression_matrix), function(i) {
      gene_expr <- expression_matrix[i,]
      if (sd(gene_expr) == 0) {
        return(NA)
...
qsave(TF.cors.SR1,"../analysis/TF.cors.SR1.qs", nthreads = cores)
qsave(TF.cors.SR2,"../analysis/TF.cors.SR2.qs", nthreads = cores)
qsave(TF.cors.SR3,"../analysis/TF.cors.SR3.qs", nthreads = cores)
```
...
```{r SR1/SR2 GSEA plots, warning=FALSE}
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.3[[target]]$targets,DEG.res=DEG.res.1,TF.name=target, comp.title="SR1/SR2 DEG")}))
...
for (i in 1:length(set1.GSEA)){
    h1 <- drawTargetHeatmap(DEG.res=DEG.res.GSEA.1, set1.GSEA[[i]]$d1$gene_name,assay.averages=assay.averages,gene_name2id=gene_name2id, gene_id2name=gene_id2name,limits.to.plot=limits.to.plot, cell.groups=c("SR1","SR2","SR3"))
...
```{r SR1/SR2L GSEA tables, warning=FALSE}
...
DEG.res.subset <- lapply(targetGenes.set.3, function(p){
  full_join(filter(DEG.res.1, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
...
combined_tibble <- combined_tibble %>%
  rowwise() %>%
  mutate(TF.cor.SR1 = TF.cors.SR1[[TF]][[ensg_id]]) %>%
  ungroup()

combined_tibble <- combined_tibble %>%
  rowwise() %>%
  mutate(TF.cor.SR2 = TF.cors.SR2[[TF]][[ensg_id]]) %>%
  ungroup()

combined_tibble <- combined_tibble %>%
  rowwise() %>%
  mutate(TF.cor.SR3 = TF.cors.SR3[[TF]][[ensg_id]]) %>%
  ungroup()
...
```{r SR2/SR3 GSEA plots, warning=FALSE, echo=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
set1.GSEA <- invisible(lapply(TF_names, function(target){do.GSEA(targets=targetGenes.set.3[[target]]$targets,DEG.res=DEG.res.2,TF.name=target,comp.title="SR2/SR3 DEG")}))
...
for (i in 1:length(set1.GSEA)){
    h1 <- drawTargetHeatmap(DEG.res=DEG.res.GSEA.2, set1.GSEA[[i]]$d1$gene_name,assay.averages=assay.averages,gene_name2id=gene_name2id, gene_id2name=gene_id2name,limits.to.plot=limits.to.plot,cell.groups=c("SR1","SR2","SR3"))
    heatmap_grob <- grid::grid.grabExpr(draw(h1))
    p1 <- set1.GSEA[[i]]$p1
  print(plot_grid(plotlist = list(p1, heatmap_grob), labels = c('A', 'B'), label_size = 12, ncol = 1, nrow = 2))
}
...
```{r SR2/SR3 GSEA tables, warning=FALSE, echo=FALSE}
tmp.1 <- lapply(targetGenes.set.3,function(p){p$targets})
names(tmp.1) <- names(targetGenes.set.3)
results <- fgsea(pathways = tmp.1, 
...
DEG.res.subset <- lapply(targetGenes.set.3, function(p){
  full_join(filter(DEG.res.2, gene_name %in% p$targets),p$target.data, by=c("gene_name"="gene_name"))
  })
