...
#Function
TAD <- lapply(rownames(E12_rV2)[1:2], function(idx){
  ...
  Link.Seurat <- LinkPeaks(object = E12_rV2, peak.assay = "peaks", expression.assay = "RNA",genes.use = idx, gene.id = T, distance = width(TAD.overlap), gene.coords = gene.range)
  
  links <- as_tibble(Link.Seurat@assays$peaks@links)[c(-1,-2,-3,-4,-5)]  %>% rename("peak"="feature") %>% rename("gene"="ensg_id")

  #links <- separate(links, col = "peak",c("seqnames", "start", "end"), sep = "-")

  dbWriteTable(con.obj, "links", links, overwrite=FALSE)
