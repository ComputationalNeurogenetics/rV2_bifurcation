...
thresholded_data <- do.call(rbind, lapply(thresholds, function(b.thr) {
    combined_tibble %>%
        filter(TF == "Tal1" & TF.cor.PRO.GA > 0.65 & TF.cor.PRO.GA < 1 & pct.1>0.1) %>%
...
Tal1.targets <- combined_tibble %>%
  filter(TF == "Tal1" & TF.cor.PRO.GA > 0.25 & pct.1 > 0.1 & sum_GA1_2_bound>0) %>%
  dplyr::select(gene_name, sum_GA1_2_bound, PRO1_2_GA_increase, TF.cor.PRO.GA)

data <- as.data.frame(Tal1.targets[,-1])
...
identify_outliers <- function(pca_data, threshold = 2) {
  pca_data %>%
    rowwise() %>%
