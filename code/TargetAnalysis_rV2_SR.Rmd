...
DEG.res.3$SR2_3_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.3$ensg_id,"SR2"]+1,base=2)
DEG.res.3$GA1_2_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.3$ensg_id,"GA1_2"]+1, base=2)
DEG.res.3$SR2_3_exp_avg <- assay.averages.merged$RNA[DEG.res.3$ensg_id,"SR2"]
DEG.res.3$GA1_2_exp_avg <- assay.averages.merged$RNA[DEG.res.3$ensg_id,"GA1_2"]
...
qsave(TF.cors.CO_GA,"../analysis/TF.cors.PRO_GA.qs", nthreads = cores)
qsave(TF.cors.CO_GL,"../analysis/TF.cors.PRO_GL.qs", nthreads = cores)
```

```{r Loading precalculated correlations}
TF.cors.CO_GA <- qread("../analysis/TF.cors.PRO_GA.qs", nthreads = cores)
TF.cors.CO_GL <- qread("../analysis/TF.cors.PRO_GL.qs", nthreads = cores)
```
...
DEG.res.subset <- lapply(targetGenes.set.SR, function(p){
  full_join(filter(DEG.res.3, ensg_id %in% p$targets_gene.id),p$target.data, by=c("ensg_id"="ensg_id"))
  })
...
```{r GA/SR GSEA plots, warning=FALSE, echo=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
...
```{r GA/SR GSEA tables, warning=FALSE, echo=FALSE}
tmp.1 <- lapply(targetGenes.set,function(p){p$targets_gene_name})
names(tmp.1) <- names(targetGenes.set)
results <- fgsea(pathways = tmp.1, 
