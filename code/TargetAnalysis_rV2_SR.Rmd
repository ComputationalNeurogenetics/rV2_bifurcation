...
```
## rV2 target tables with GA1_2 and GL1_2 information - Tal1 motifs separated

```{r rV2 target tables with GA1_2 and GL1_2 information Tal1 motif0}
motif0.targets <- getTargets_rV2_TFBS_level("Tal1",con, motif="TAL1.H12CORE.0.P.B_TAL1.H12CORE.0.P.B")
targetGenes.set<-list(motif0.targets)
names(targetGenes.set) <- "Tal1"
DEG.res.subset <- lapply(targetGenes.set, function(p){full_join(filter(DEG.res.1, ensg_id %in% p$targets_gene.id) %>% dplyr::select(-gene_name),p$target.data, by=c("ensg_id"="ensg_id"))})
combined_tibble <- map_dfr(DEG.res.subset, ~ .x, .id = "TF")
...
combined_tibble <- combined_tibble %>% mutate_if(is.numeric, round, digits=3)
combined_tibble -> Tal1_motif0_targets
create_dt(Tal1_motif0_targets)
```
```{r rV2 target tables with GA1_2 and GL1_2 information Tal1 motif1}
motif1.targets <- getTargets_rV2_TFBS_level("Tal1",con, motif="TAL1.H12CORE.1.P.B_TAL1.H12CORE.1.P.B")
targetGenes.set<-list(motif1.targets)
names(targetGenes.set) <- "Tal1"
DEG.res.subset <- lapply(targetGenes.set, function(p){full_join(filter(DEG.res.1, ensg_id %in% p$targets_gene.id) %>% dplyr::select(-gene_name),p$target.data, by=c("ensg_id"="ensg_id"))})
combined_tibble <- map_dfr(DEG.res.subset, ~ .x, .id = "TF")
...
combined_tibble <- combined_tibble %>% mutate_if(is.numeric, round, digits=3)
combined_tibble -> Tal1_motif1_targets
create_dt(Tal1_motif1_targets)
```

```{r rV2 target tables with GA1_2 and GL1_2 information Tal1 motif2}
motif2.targets <- getTargets_rV2_TFBS_level("Tal1",con, motif="TAL1.H12CORE.2.P.B_TAL1.H12CORE.2.P.B")
targetGenes.set<-list(motif2.targets)
names(targetGenes.set) <- "Tal1"
DEG.res.subset <- lapply(targetGenes.set, function(p){full_join(filter(DEG.res.1, ensg_id %in% p$targets_gene.id) %>% dplyr::select(-gene_name),p$target.data, by=c("ensg_id"="ensg_id"))})
combined_tibble <- map_dfr(DEG.res.subset, ~ .x, .id = "TF")
...
combined_tibble <- combined_tibble %>% mutate_if(is.numeric, round, digits=3)
combined_tibble -> Tal1_motif2_targets
create_dt(Tal1_motif2_targets)
```

## Tal1 FT to correlation 
