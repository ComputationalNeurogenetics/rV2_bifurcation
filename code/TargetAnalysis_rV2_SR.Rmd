...
cores <- 6
con <- DBI::dbConnect(RSQLite::SQLite(), dbname = "../../TOBIAS.dr.h12_140524.sqlite")
con.sr <- DBI::dbConnect(RSQLite::SQLite(), dbname = "../../TOBIAS.serot.h12_230524.sqlite")
...
qsave(merged.datasets, file="merged.datasets.qs",nthreads = 10)
qsave(gene_id2name,file="gene_id2name.qs")
qsave(gene_name2id,file="gene_name2id.qs")
```

```{r}
merged.datasets <- qread(file="merged.datasets.qs",nthreads = 10)
gene_id2name <- qread(file="gene_id2name.qs")
gene_name2id <- qread(file="gene_name2id.qs")
```
...
Idents(merged.datasets) <- merged.datasets$merged.groups
DEG.res.3 <- FindMarkers(merged.datasets, ident.1="SR1", ident.2 = "SR2",logfc.threshold=0, assay = "RNA", slot="data", mean.fxn=mean.fxn) %>% rownames_to_column(var="ensg_id") %>% as_tibble()

colnames(assay.averages.merged$RNA) <- str_replace_all(colnames(assay.averages.merged$RNA), pattern = "-", replacement = "_")

DEG.res.3$SR2_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.3$ensg_id,"SR2"]+1,base=2)
DEG.res.3$SR1_exp_avg_log2 <- log(assay.averages.merged$RNA[DEG.res.3$ensg_id,"SR1"]+1, base=2)
DEG.res.3$SR1_exp_avg <- assay.averages.merged$RNA[DEG.res.3$ensg_id,"SR1"]
...
```{r Correlations between target genes and selector genes, warning=FALSE, eval=FALSE}
TF_names <- c("Tal1","Gata2","Gata3","Vsx2")
...
qsave(TF.cors.CO_GA,"../analysis/TF.cors.CO_GA.qs", nthreads = cores)
qsave(TF.cors.CO_GL,"../analysis/TF.cors.CO_GL.qs", nthreads = cores)
```

```{r Loading precalculated correlations}
TF.cors.CO_GA <- qread("../analysis/TF.cors.CO_GA.qs", nthreads = cores)
TF.cors.CO_GL <- qread("../analysis/TF.cors.CO_GL.qs", nthreads = cores)
```
...
DEG.res.subset <- lapply(targetGenes.set, function(p){
  full_join(filter(DEG.res.1, ensg_id %in% p$targets_gene.id),select(p$target.data,-gene_name), by=c("ensg_id"="ensg_id"))
  })
...
## Serot target tables with SR1, SR2 and SR3 information

```{r}
DEG.res.subset <- lapply(targetGenes.set.SR, function(p){
  full_join(filter(DEG.res.3, ensg_id %in% p$targets_gene.id),  select(p$target.data,-gene_name), by=c("ensg_id"="ensg_id"))
  })
