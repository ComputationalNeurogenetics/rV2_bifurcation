...
consensus.beds <- lapply(consensus.beds.path, function(path){
  tmp.1 <- read_delim(file = path, col_names = c("chr","start","end","start_merged","end_merged","total_signals","max_signals","max_signal_regions","sample_names","consensus_replicate_count"),col_types = cols(
      chr = col_character(),
      start = col_integer(),
      end = col_integer(),
      start_merged = col_character(),  # Force as integer
      end_merged = col_character(),    # Force as integer
      total_signals = col_character(),
      max_signals = col_character(),
      max_signal_regions = col_character(),
      sample_names = col_character(),
      consensus_replicate_count = col_integer()))
  if (nrow(tmp.1)>0){
...
normalized_data <- consensus.beds.tb %>%
  ...
  separate_rows(start_merged,end_merged, total_signals, max_signals, max_signal_regions,sample_names, sep = ",",convert = TRUE) %>%
    mutate(
    start_merged = as.integer(start_merged),
    end_merged = as.integer(end_merged)
  ) %>%
