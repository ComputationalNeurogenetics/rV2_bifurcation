...
total_signals_table <- consensus.beds.tb %>%
  dplyr::select(chr,start,end, total_signals) %>%
  separate_rows(total_signals, sep = ",") %>%
  mutate(tag_id = row_number()) # Create unique IDs for tags


expanded_data <- consensus.beds.tb %>%
  separate_rows(total_signals, sep = ",") %>%
  left_join(total_signals_table, by = "total_signals")
