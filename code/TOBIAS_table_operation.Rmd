...
qsave(tobias.tb, file = "../analysis/Tobias.rv2.h12.dr.tb.qs",nthreads = cores)
```
...
calculate_weighted_mean <- function(data) {
  ...
  if (length(weights) != nrow(data)) {
    ...
    problematic_groups <<- append(problematic_groups, list(tibble(start = unique(data$start), end = unique(data$end), TFBS_name = TFBS_name, mean.cons = mean(data$score))))
    # Return NA
    mean.cons <- NA
  } else {
...
TFBS.loc.cons.list <- mclapply(uniq.seq.names, function(seq.name){
  ...
  TFBS.cons <- mergeByOverlaps(query = cons.subset, subject = TFBS.tmp, type="within")
  mcols(TFBS.cons$TFBS.tmp)$score <- TFBS.cons$score
  TFBS.df <- as.data.frame(TFBS.cons$TFBS.tmp)
