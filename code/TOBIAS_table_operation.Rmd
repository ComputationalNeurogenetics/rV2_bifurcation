...
con <- DBI::dbConnect(RSQLite::SQLite(), dbname = "~/Workspace/TOBIAS.dr.h12_130324.sqlite")
```
...
for (ensg_id in TF_ensg_ids){
  tryCatch({
    exp.data.matrix <- FetchData(dataset, vars = ensg_id)
    test.res <- cor(exp.data.matrix,acc.data.matrix, method = "spearman", use="pairwise.complete.obs")
    data.to.db <- tibble(ensg_id=rownames(test.res),cor=test.res[1,])
    dbWriteTable(con, "gene2feat_cor", data.to.db, overwrite=FALSE, append=TRUE)
  }, error=function(cond){
    cond
...
dbExecute(con, 'CREATE INDEX gene2feat_cor_ensg_id ON ensg_id (gene2feat_cor);')
dbExecute(con, 'CREATE INDEX gene2feat_cor_cor ON cor (gene2feat_cor);')
```
