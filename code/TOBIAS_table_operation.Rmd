...
rV2.groups.tobias.h12.gr.dr <- get_BINDetect_snakemake_results_gr(res_path="/Volumes/MyBookDuo/Data/TOBIAS_input_output/E12rV2_090224/TFBS/",parallel=T, mc.cores=2, HOCOMOCO=12)
...
```{r}
tobias.tb <- qread(file = "../analysis/Tobias.rv2.h12.dr.tb.qs",nthreads = cores)
tobias.tb$start <- tobias.tb$start+1 
```
...
qsave(tobias.tb.2, file = "../analysis/Tobias.rv2.h12.dr.tb.2.qs",nthreads = cores)
```
...
# TODO: problem is that motif coordinates are not as whole in tobias results. We need whole coordinates there!!
...
TFBS.df <- TFBS.df %>%
  ...
  mutate(is_consecutive = (start == dplyr::lag(end, default = dplyr::first(start)) + 1) & (TFBS_name == dplyr::lag(TFBS_name, default = dplyr::first(TFBS_name))), group = cumsum(!is_consecutive)) %>%
