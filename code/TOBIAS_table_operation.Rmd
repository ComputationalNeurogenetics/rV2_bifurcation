...
gene_positions <- as_tibble(Annotation(dataset)) %>%
  ...
  group_by(gene_id) %>%
  summarise(
    seqnames = first(seqnames), # Ensure one seqname per gene_id
    strand = first(strand),     # Ensure one strand per gene_id
    gene_start = if_else(first(strand) == "-", max(end, na.rm = TRUE), min(start, na.rm = TRUE)),
    gene_end = if_else(first(strand) == "-", min(start, na.rm = TRUE), max(end, na.rm = TRUE)),
    .groups = "drop"
  ) %>%
  distinct(gene_id, .keep_all = TRUE)  # Ensure unique rows by gene_id
