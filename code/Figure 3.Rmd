...
con <- DBI::dbConnect(RSQLite::SQLite(), dbname = "~/Workspace/TOBIAS.dr.h12_2.sqlite")
...
maxes <- find.maxes(features=Tal1.features, dbname="~/Workspace/TOBIAS.dr.h12_2.sqlite")

for (i in 1:nrow(Tal1.features)){
  p <- plotHorizDotplot_v2(feature.coords = as.character(Tal1.features[i,2]), max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp)
  if(is.list(p)){
...
maxes <- find.maxes(features=Tal1.features)

igv<-igvR()
for (i in 1:nrow(Tal1.features)){
  p <- plotHorizDotplot_v3(feature.coords = as.character(Tal1.features[i,2]), igv=igv, cons.filt=cons.gr, max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,acc.thr=acc.thr)
  if(is.list(p)){
...
maxes <- find.maxes(features=Vsx2.features)
for (i in 1:nrow(Vsx2.features)){
  p <- plotHorizDotplot_v2(feature.coords = as.character(Vsx2.features[i,2]),max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp)
  if(is.list(p)){
...
igv<-igvR()
for (i in 1:nrow(Vsx2.features)){
  p <- plotHorizDotplot_v3(feature.coords = as.character(Vsx2.features[i,2]), igv=igv, cons.filt=cons.gr,max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,acc.thr=acc.thr)
  if(is.list(p)){
  pdf(file=paste(plot_loc_vsx2,paste("Vsx2f",i,"_dotplot_v3.pdf",sep=""),sep=""), width = 15, height = 12)
    plot.to.plot <- p$p3 + plot_spacer() + p$p1 + p$p2 + plot_layout(ncol=2,nrow = 2,byrow = TRUE, widths = c(10,.5), heights = c(3,1))
  print(plot.to.plot)
  dev.off()
  }
...
maxes <- find.maxes(features=Gata2.features)
for (i in 1:nrow(Gata2.features)){
  p <- plotHorizDotplot_v2(feature.coords = as.character(Gata2.features[i,2]),max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp)
  if(is.list(p)){
...
igv<-igvR()
for (i in 1:nrow(Gata2.features)){
  p <- plotHorizDotplot_v3(feature.coords = as.character(Gata2.features[i,2]), igv=igv, cons.filt=cons.gr,max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,acc.thr=acc.thr)
  if(is.list(p)){
...
for (i in 1:nrow(Gata3.features)){
  p <- plotHorizDotplot_v2(feature.coords = as.character(Gata3.features[i,2]),max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp)
  if(is.list(p)){
...
igv<-igvR()
for (i in 1:nrow(Gata3.features)){
  p <- plotHorizDotplot_v3(feature.coords = as.character(Gata3.features[i,2]), igv=igv, cons.filt=cons.gr,max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,acc.thr=acc.thr)
  if(is.list(p)){
