...
dbname="~/Workspace/TOBIAS.dr.h12_210324.sqlite"
...
maxes <- find.maxes(features=Vsx2.features, dbname=dbname)
for (i in 1:nrow(Vsx2.features)){
  p <- plotHorizDotplot_v2(feature.coords = as.character(Vsx2.features[i,2]),max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,dbname=dbname)
  if(is.list(p)){
...
igv<-igvR()
for (i in 1:nrow(Vsx2.features)){
  p <- plotHorizDotplot_v3(feature.coords = as.character(Vsx2.features[i,2]), igv=igv, cons.filt=cons.gr,max.acc=maxes$max.acc, max.exp = maxes$max.exp, max.fp=maxes$max.fp,acc.thr=acc.thr, dbname=dbname)
  if(is.list(p)){
