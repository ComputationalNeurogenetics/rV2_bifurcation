...
```{r Tal1f6}
Tal1f6 <- filter(tobias.table, features==tal1.feat6) %>% left_join(exp.table) %>% left_join(acc.table, by=c("features"="features"))

Tal1f6.data <- Tal1f6 %>% collect()

#fp.mean.by.motif <- Tal1f6.data %>% group_by(TFBS_name) %>% summarize(mean.fp=mean(mean(PRO1_2_score),mean(CO1_2_score), mean(GA1_2_score),mean(GL1_2_score)))

Tal1f6.data.filt <- Tal1f6.data %>% filter((abs(PRO1_2.x)>1.5 | abs(CO1_2.x)>1.5 | abs(GA1_2.x)>1.5) & (PRO1_2_bound==1 | CO1_2_bound==1 | GA1_2_bound==1) & mean_cons>.5) %>% arrange(start)

# Replace cases where TF motif repeats and overlaps itself with average row.

Tal1f6.data.filt$TFBS_name_comb <- paste(str_remove(string = Tal1f6.data.filt$TFBS_name, pattern = "_.*")," (",Tal1f6.data.filt$start,"-",Tal1f6.data.filt$end,")",sep="")
Tal1f6.data.filt$TFBS_name_comb <- factor(Tal1f6.data.filt$TFBS_name_comb, levels = Tal1f6.data.filt$TFBS_name_comb)
...
p.1 <- ggplot(Tal1f6.data.filt) + 
    geom_point(aes(x = TFBS_name_comb, fill=PRO1_2.x, y="4",
                   shape=as.character(PRO1_2_bound)), size=5) +
  geom_point(aes(x = TFBS_name_comb, fill=CO1_2.x, y="3",
                 shape=as.character(CO1_2_bound)), size=5) + 
  geom_point(aes(x = TFBS_name_comb, fill=GA1_2.x, y="2",
                 shape=as.character(GA1_2_bound)), size=5) +
  geom_point(aes(x = TFBS_name_comb, fill=GL1_2.x, y="1",
                 shape=as.character(GL1_2_bound)), size=5) +
scale_fill_gradient2(low="blue", mid="gray",high="red", midpoint = 0) + theme_minimal() + theme(axis.text.x = element_text(size = 12, angle = 90)) + ylab("Cell group") + scale_shape_manual(values=c(4,22) , guide = "none") + scale_y_discrete("Cell group", labels=c("4"="PRO1_2","3"="CO1_2","2"="GA1_2","1"="GL1_2")) + labs(fill="Expression z-score")
  
  
  scale_x_discrete("TF-motif", labels=Tal1f6.data.filt$TFBS_name) + 
