...
test <- filter(tobias.table, features==tal1.feat6) %>% left_join(exp.table) %>% left_join(acc.table, by=c("features"="features"))

test %>% show_query()

test.data <- test %>% collect()

fe2 <- filter(test.data, mean_cons>0.5 & (PRO1_2.x>0 | CO1_2.x>0 | GA1_2.x>0) & (PRO1_2_score>0 | CO1_2_score>0 | GA1_2_score>0))
...
fp.mean.by.motif <- test.data %>% group_by(TFBS_name) %>% summarize(mean.fp=mean(mean(PRO1_2_score),mean(CO1_2_score), mean(GA1_2_score),mean(GL1_2_score)))

test.data.filt<- left_join(test.data,fp.mean.by.motif) %>% filter((PRO1_2.x>0.5 | CO1_2.x>0.5 | GA1_2.x>0.5) & (PRO1_2_bound==1 | CO1_2_bound==1 | GA1_2_bound==1)) %>% filter(mean_cons>.5)
