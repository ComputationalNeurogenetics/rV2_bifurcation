...
e.all.combined <- readRDS("../scRNA_data/e_all_rescaled_regress_nuisance.Rds")
```
...
# Convert symbols to ENSMUS IDs based on data in E12 (WLOG) RNA assay.
...
```{r}
e12.rv2.ss <- subset(e.all.combined[[1]],
                   idents = c(4, 12, 19, 22, 24, 25, 17, 5))
```

```{r}
# Outlier cells to be removed
rv2.clean <- readRDS("../data/e12_rv2_clean_subset_310323.rds")
```

```{r}
e12.rv2.ss <- subset(e12.rv2.ss, cells = rv2.clean)
```

```{r}
DimPlot(e12.rv2.ss, pt.size = 1.3)

ggsave("e12_rv2_subset.pdf",
       plot = last_plot(),
       device = "pdf",
       path = "../figures/",
       width = 12,
...
```{r}
rv2.key.symb <- c("Nkx6-1", "Tal1", "Vsx2", "Gata2", "Slc17a6", "Gad1")
rv2.key.ids <- convert_feature_identity(e.all.combined[[1]], assay = "RNA",
                                         features = rv2.key.symb, feature.format = "symbol")
```

```{r}
e12.fp.extra <- FeaturePlot(e12.rv2.ss, features = rv2.key.ids, combine = F,
                       label = F, pt.size = cell.size)

patchwork::wrap_plots(
  lapply(1:length(e12.fp.extra), function(i) {
    e12.fp.extra[[i]] + ggtitle(rv2.key.symb[i]) + coord_fixed() + theme(aspect.ratio=1)
  }), nrow = 3
)

ggsave("e12_key_markers_310323_5.pdf",
       plot = last_plot(),
       device = "pdf",
       path = "../figures/",
       width = 12,
...
e12.gaba <- subset(e.all.combined[[1]],
                   idents = c(4, 12, 19, 24, 25, 26))
e13.gaba <- subset(e.all.combined[[2]],
                   idents = c(7, 8, 10, 21, 28))
e14.gaba <- subset(e.all.combined[[3]],
                   Mixed_cells %in% c("4", "7", "9", "15", "25", "0_2"))
e15.gaba <- subset(e.all.combined[[4]],
                   Mixed_cells %in% c("4", "6", "13", "18", "32", "37", "41", "44", "0_2"))
```
...
#
# Reflecting the changes in marker score filtering, now loops through the set of n values, as well.
