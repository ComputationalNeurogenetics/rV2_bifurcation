...
#min.dplyr::filter <- (assay.averages$RNA[,"GA1_2"]>0.1 | assay.averages$RNA[,"GA1_2"]==0) & (assay.averages$RNA[,"GL1_2"]>0.1 | assay.averages$RNA[,"GL1_2"]==0)
...
DEG.res <- FindMarkers(rV2.dataset, ident.1="GA1_2", ident.2 = "GL1_2",logfc.threshold=0, assay = "RNA", slot="data") %>% rownames_to_column(var="ensg_id") %>% as_tibble() %>% dplyr::filter(p_val_adj<0.01)
...
# DEG.res$avg_log2FC <- dplyr::filter(DEG.res, p_val_adj<0.05) %>% pull(avg_log2FC)
# names(DEG.res$avg_log2FC) <- dplyr::filter(DEG.res, p_val_adj<0.05) %>% pull(ensg_id)
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Tal1$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Gata2$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Gata3$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Vsx2$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Sox4$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Klf17$gene_name))
...
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Myc$gene_name))
...
## With C&T included in conditions (no conservation score thr)
...
# target.genes.in.order <- names(sort(DEG.res.GSEA[targets.Tal1$gene_name], decreasing = TRUE))
# Targets.expression.avg <- t(assay.averages$RNA[sapply(target.genes.in.order,function(id){gene_name2id[[id]]}),c("GA1_2","GL1_2")])
# colnames(Targets.expression.avg) <- sapply(colnames(Targets.expression.avg),function(id){gene_id2name[[id]]})
# col_fun = colorRamp2(c(min(Targets.expression.avg), max(Targets.expression.avg)), c("white", "red"))
# Heatmap(Targets.expression.avg, cluster_columns = FALSE,heatmap_width = unit(12, "inches"), col=col_fun)

create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Tal1$gene_name))
#create_dt(Targets.expression.avg)

TF_name<-"Gata2"
print(TF_name)
targetQuery <- paste("SELECT gm.gene_name  FROM links as li, tobias as tb, CT_data as ct, gene_metadata as gm WHERE gm.ensg_id=li.ensg_id AND tb.features=li.feature AND tb.TF_gene_name='",toupper(TF_name),"' AND li.zscore>2 AND li.pvalue<0.001 AND ct.target_gene_name='",TF_name,"' AND (tb.GA1_2_bound=1 OR tb.GL1_2_bound=1) AND ct.feature=tb.features;",sep="") 
targets.Gata2 <- as_tibble(dbGetQuery(con, targetQuery)) %>% distinct()
plotEnrichment(targets.Gata2$gene_name, DEG.res.GSEA) + ggtitle(paste(TF_name,"target enrichments in GA/GL DEG")) + geom_rect(aes(xmin=0,xmax=lower.pos,ymin=-0.05,ymax=0.05), fill="blue", alpha=0.002)+ geom_rect(aes(xmin=mid.lower.index,xmax=mid.upper.index,ymin=-0.05,ymax=0.05), fill="green", alpha=0.002)+ geom_rect(aes(xmin=upper.pos,xmax=nrow(DEG.res),ymin=-0.05,ymax=0.05), fill="red", alpha=0.002)
create_dt(DEG.res %>% dplyr::filter(gene_name %in% targets.Gata2$gene_name))
...
## With C&T included in conditions (all Tal1 motifs)
