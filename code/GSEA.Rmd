...
```{r}
con.obj <- DBI::dbConnect(RSQLite::SQLite(), dbname = "../../TOBIAS.dr.h12_030324.sqlite")
```
...
rV2.dataset <- qread("../scATAC_data/nmm_rV2_subset_relabeled_031023_links.qs", nthreads = 3)

serot.dataset <- qread("../serot/sero_relabeled_201123_links.qs", nthreads = 3)
```
...
TF_name<-"Tal1"
targetQuery <- paste("SELECT li.ensg_id FROM links as li, tobias as tb, CT_data as ct WHERE tb.features=li.feature AND tb.TF_gene_name='",toupper(TF_name),"' AND li.zscore>2 AND tb.GA1_2_bound=1 AND li.pvalue<0.001 AND ct.target_gene_name='",TF_name,"' AND ct.feature=tb.features;",sep="") 
targets.Tal1 <- as_tibble(dbGetQuery(con, targetQuery)) %>% distinct()

TF_name<-"Vsx2"
targetQuery <- paste("SELECT li.ensg_id FROM links as li, tobias as tb, CT_data as ct WHERE tb.features=li.feature AND tb.TF_gene_name='",toupper(TF_name),"' AND li.zscore>2 AND tb.GL1_2_bound=1 AND li.pvalue<0.001 AND ct.target_gene_name='",TF_name,"' AND ct.feature=tb.features;",sep="") 
targets.Vsx2 <- as_tibble(dbGetQuery(con, targetQuery)) %>% distinct()

geneSets <- list(targets.Tal1=targets.Tal1$ensg_id, targets.Vsx2=targets.Vsx2$ensg_id, GABA.genes=GABA.genes, GLU.genes=GLUT.genes)
...
targetQuery <- paste("SELECT li.ensg_id FROM links as li, tobias as tb, CT_data as ct WHERE tb.features=li.feature AND tb.TF_gene_name='",toupper(TF_name),"' AND li.zscore>1.5 AND tb.GA1_2_bound=1 AND li.pvalue<0.001 AND ct.target_gene_name='",TF_name,"' AND ct.feature=tb.features;",sep="") 
